...
class UIManager:
    ...
    def setup_table(table, rows, cols, headers_x=None, headers_y=None):
        ...
        if headers_x: table.setHorizontalHeaderLabels(headers_x)
        if headers_y: table.setVerticalHeaderLabels(headers_y)
    ...
    def auto_set_height(table, max_h=500):
        # Výpočet potřebné výšky: hlavička + všechny řádky + okraj
        h = table.horizontalHeader().height()
        for r in range(table.rowCount()):
            h += table.rowHeight(r)
        
        # Připočítáme malou rezervu pro grid/okraj
